...
class KernelSVC:
    ...
    def fit(self, K, y, class_weights=None):
        ...
        assert (np.unique(y) == np.array([-1, 1])).all(), print(
        ...
        # Margin Points
        support_idx = np.where(
            (self.alpha < C - self.epsilon) & (self.alpha > self.epsilon)
        )

        # Offset for the classifier
        self.b = np.median(
            y[support_idx] - K[support_idx] @ np.multiply(self.alpha, self.y)
        )
...
def stratified_cross_val(
    G_train, train_labels, G_test, C, class_weights, n_fold=6, seed=42, verbose=False
):
    ...
    for i in range(n_fold):
        ...
        if verbose:
            print(
                f"Percentage of positive values: {np.sum(y_train_fold)/len(y_train_fold):%}%"
            )
...
def sorted_neighbors_id(G, vertex_id, pattern_dict):
    ...
    neighbors_dict_sorted = dict(
        sorted(
            neighbors_dict.items(), key=lambda item: unique_pattern_order.index(item[1])
        )
    )
...
def WL_features(Graph, max_iter, verbose=False):
    ...
    pattern_dict = {
        str(pattern): list(atom_types.values()).count(pattern)
        for pattern in atom_types_set
    }
...
def clean_dataset(dataset, labels, discard=True):
    ...
    for mol_id in range(len(dataset)):
        ...
        if len(list(nx.connected_components(dataset[mol_id]))) > 1:
            ...
            if (
                len(sorted(list(nx.connected_components(dataset[mol_id])), key=len)[-2])
                == 1
            ):
